...
if(isset($_POST)&& !empty($_POST)){
    ...
    $id_commande=$_POST['com-id'];
    //update old events
    foreach ($_POST['name-event'] as $key => $oldevent) {
       
        $old_event_name=$oldevent;
        $old_event_price= $_POST['price'][$key];
        $old_event_quantity=$_POST['quantity'][$key];
        $old_event_id=$_POST['name-id'][$key];    
        $commandeC->updateCommandeOldEvent($old_event_id,$id_commande,$old_event_quantity,$old_event_price,$old_event_name);
   ...
   //add new events
   foreach ($_POST['new_event_name'] as $key => $newevent) {

    $new_event_quantity= $_POST['new_event_price'][$key];
    $new_event_price=$_POST['new_event_quantity'][$key];

    $commandeC-> addCommandeNewEvent($id_commande,$new_event_quantity,$new_event_price,$newevent);

    $total+=$_POST['new_subtotal'][$key];
   ...
   //updateuser
   $commandeC->updateCommandeUser($id_user,$login,$email);
   //update commande price
   $commandeC->updateCommandeprice($total,$id_commande);
...
<body class="app sidebar-mini ltr light-mode">
            ...
            <div class="main-content app-content mt-0">
                <div class="side-app">
                    ...
                    <div class="main-container container-fluid">
                        ...
                        <?php $comid=isset($_GET["order_id"]) ?  $_GET['order_id'] : ''?>
                        <!-- Row -->
                            ...
                            <div class="col-lg-12">
                                <div class="card">
                                    ...
                                    <form method="post" >
                                        ...
                                        <input type="text" class="form-control" id="name" name="name" value="<?php echo $userinfos['login']; ?>">
                                        <input type="hidden" class="form-control" id="user-id" name="user-id" value="<?php echo $userinfos['id']; ?>">
                                        </div>
                                        <div class="form-group">
                                        <label for="email">Email:</label>
                                        <input type="email" class="form-control" id="email" name="email" value="<?php echo $userinfos['email']; ?>">
                                        <?php 
                                        $date_for_input = date("Y-m-d", strtotime($userinfos['ddn']));?> 
                                        <input type="hidden" class="form-control" id="ddn" name="ddn" value="<?php echo $date_for_input; ?>" disabled>
                                        </div>
                                        ...
                                        <table class="table table-bordered" id="event-table">
                                            ...
                                            foreach($commandeEvents as $item) { ?>
                                                <tr>
                                                <td><input type="text" class="form-control name-input" name="name-event[]" value="<?php echo $item['name']; ?>" style="text-align:center">
                                                <input type="hidden" class="form-control" id="com-id" name="com-id" value="<?php echo $comid ;?>" style="text-align:center">
                                                <input type="hidden" class="form-control name-input" name="name-id[]" value="<?php echo $item['id_event']; ?>" style="text-align:center">
                                                 </td>
                                                <td><input type="number" class="form-control price-input" name="price[]" value="<?php echo $item['price']; ?>" style="text-align:center"></td>
                                                <td><input type="number" class="form-control quantity-input" name="quantity[]" value="<?php echo $item['quantity_event']; ?>" style="text-align:center"></td>
                                                
                                                <td><input type="number" class="form-control subtotal-input"  name="subtotal[]" value="<?php echo $item['price'] * $item['quantity_event']; ?>" style="text-align:center"> </td>
                                                <td>
                                                    <button class="delete-btn" onclick="deleteProduct(this.parentNode.parentNode)">Delete</button>
                                                </td>
                                                </tr>
                                            <?php } ?>
                                            <button id="add-event-btn" type="button" class="btn btn-secondary"  style="background-color: red; color: white;" onclick="addRow()" >Add New Event</button>
   ...
   /* const addEventBtn = document.getElementById("add-event-btn");
    const newEventRow = document.getElementById("new-event-row");

    addEventBtn.addEventListener("click", (e) => {
        e.preventDefault();
    if (newEventRow.style.display === "none") {
        newEventRow.style.display = "table-row";
    } else {
        newEventRow.style.display = "none";
    }
    });*/
    ...
    // Calculate and update the subtotal when the quantity changes
    quantityInput.addEventListener('change', function() {
        const quantity = parseFloat(quantityInput.value);
        const price =  parseFloat(priceInput.value);; // Replace with your product price
        const subtotal = quantity * price;
        subtotalInput.value = subtotal.toFixed(2); // Format the subtotal to two decimal places
    });
    ...
    /*var newquantityInput = document.querySelector('.new_event_quantity');
    
    var newpriceInput = document.querySelector('.new_event_price');
    var newsubtotalInput = document.querySelector('.new_subtotal');

    // Calculate and update the subtotal when the quantity changes
    newquantityInput.addEventListener('change', function() {
        var quantity = parseFloat(newquantityInput.value);
        var price =  parseFloat(newpriceInput.value);; // Replace with your product price
        var subtotal = quantity * price;
        newsubtotalInput.value = subtotal.toFixed(2); // Format the subtotal to two decimal places
    });

    newpriceInput.addEventListener('change', function() {
        var quantity = parseFloat(newquantityInput.value);
        var price =  parseFloat(newpriceInput.value);; // Replace with your product price
        var subtotal = quantity * price;
        newsubtotalInput.value = subtotal.toFixed(2); // Format the subtotal to two decimal places
    });*/
    document.getElementById('event-table').addEventListener('change', function(event) {
        
        if (event.target.classList.contains('new_event_quantity')) {
            console.log('new_event_quantity');
            var row = event.target.parentNode.parentNode;
            console.log(row.querySelector('.new_event_price').value);
            var price = parseFloat(row.querySelector('.new_event_price').value);
            console.log(price);
            var quantity = parseFloat(event.target.value);
            row.querySelector('.new_subtotal').value = (price * quantity).toFixed(2);
        
        }

        if (event.target.classList.contains('new_event_price')) {
            var row = event.target.parentNode.parentNode;
            var quantity = parseFloat(row.querySelector('.new_event_quantity').value);
  ...
  // Get a reference to the table
        ...
        cell1.innerHTML = `<input type="text" name="new_event_name[]" style="text-align:center"  required>`;
        cell2.innerHTML = `<input type="number" name="new_event_quantity[]" class="new_event_quantity" style="text-align:center">`;
        cell3.innerHTML = `<input type="number" name="new_event_price[]" class="new_event_price"  style="text-align:center">`;
        cell4.innerHTML = `<input type="number" name="new_subtotal[]" class="new_subtotal" style="text-align:center">`;
        cell5.innerHTML = `<button class="delete-btn" onclick="deleteProduct(this.parentNode.parentNode)">Delete</button>`;    
